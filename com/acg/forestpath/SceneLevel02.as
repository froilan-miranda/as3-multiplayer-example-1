...
public class SceneLevel02 extends Sprite
{
	...
	//private var waitOnHighScore:int = 3000;
	private var completeTimeoutId:int;
	//private var highScoreTimeoutId:int;
	...
	public function SceneLevel02():void
	{
		trace("Level 02 initiated");
		this.alpha = 0.0;
		if(stage)
			loadSettings(null);
		else
	...
	private function initScene():void
	{
		...
		playerStations = []; //initiate stations array t
	...
	private function onLevelComplete():void
	{
		...
		trace("level complete box added" + levelComplete.y + "|" + levelComplete.x);
	...
	{
		ssDebug.trace("a second has passed");
		for each(var pStation:PlayerStation in playerStations){
	...
	{
		spawnManager = new SpawnManager(2);
	...
	private function createPlayerStations():void
	{
		...
		for each(var index:PlayerInfo in PlayerInfoManager.activePlayers){
			...
			ssDebug.trace(index + " | " +  index.position + " | "  + reticleManager.jStickAtlas[index.position - 1]);
	...
	private function processJoystickData(e:CustomEvent):void
	{
		...
		for (var key:Object in activeReticles){
			//ssDebug.trace(activeReticles[key]);
